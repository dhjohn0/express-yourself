...
module.exports = (db, search, config) => {
  ...
  if (search) {
    search.paginate = async (searchTerm, type, start, length, sort) => {
      ...
      let opts = {
        index: `${config.search.index}.${type}`,
        q: searchTerm.indexOf(':') >= 0 ? `${type}.${searchTerm}` : `*:${searchTerm}`,
        from: start,
        size: length,
        sort: `${type}.${sort.column}:${sort.desc ? 'desc' : 'asc'}`
      };
      ...
      let p = {
        list: results.hits.hits.map(v => {
          v._source[type]._id = v._id;
          return v._source[type];
        }),
        pagination: {
          previous: start - length,
          start: start + 1,
